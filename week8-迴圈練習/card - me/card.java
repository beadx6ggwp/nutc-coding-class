...
class card {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 4; i++) {
            sum = ArrraySum2(players[i]);
            if (sum > max) {
                max = sum;
                ind = i;
            }
        }
        int temp[] = players[win];
        players[win] = players[ind];
        players[ind] = temp;
        ...
        System.out.println("每副牌的大小, 設定贏家:" + win);
        for (int i = 0; i < 4; i++) {
            System.out.println(ArrraySum2(players[i])+", "+ArrraySum(players[i]));
        }
    }
    ...
    public static int ArrraySum2(int[] arr) {
        int count = 0, convert = 0;
        for (int i = 0; i < arr.length; i++) {
            convert = (arr[i] - 1) % 13;
            if (convert >= 10){
